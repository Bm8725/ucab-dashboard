...
export default function UcabRiders() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-[400px] bg-[#0c0c0c]">
    ...
    <div className="w-full bg-[#0c0c0c] min-h-screen text-[#e0e0e0] font-sans p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
          ...
          UCAB RIDERS{' '}
          <span className="text-[#ff007a] font-black text-[10px] bg-[#ff007a]/10 px-2 py-1 rounded uppercase tracking-widest">
            CEO_PANEL
          </span>
        </h1>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#ff007a] hover:bg-[#d00063] text-white px-4 py-2 rounded-lg text-xs font-black uppercase tracking-tighter transition-all"
          >
            ...
            {currentItems.map((rider: Rider) => (
              <tr key={rider.id} className="hover:bg-[#151515] transition-colors">
                <td className="p-5">
                  ...
                  <div className="text-[9px] text-[#444] font-mono mt-1 uppercase tracking-tighter italic">
                    ID: {rider.id.slice(0, 12)}
                  </div>
                </td>
                <td className="p-5">
                  <div className="flex flex-col gap-1 text-[12px] text-[#999]">
                    <span className="flex items-center gap-2">
                      <Mail size={12} className="text-[#ff007a] opacity-70" /> {rider.email}
                    </span>
                    <span className="flex items-center gap-2 text-[#555]">
                      <MapPin size={12} /> {rider.address || 'Nespecificată'}
                    </span>
                  </div>
                </td>
                ...
                <td className="p-5 text-right px-10">
                  <div className="flex justify-end gap-3 items-center">
                    <button onClick={() => setEditingRider(rider)} className="text-[#444] hover:text-[#ff007a] transition-all">
                      <Edit2 size={16} />
                    </button>
                    {deleteConfirm === rider.id ? (
                      <div className="flex items-center gap-2 bg-[#ff007a]/10 p-1 rounded border border-[#ff007a]/20">
                        <span className="text-[9px] font-black text-[#ff007a] px-1">CONFIRMĂ?</span>
                        <button onClick={() => handleDelete(rider.id)} className="bg-[#ff007a] text-white p-1 rounded hover:bg-red-600 transition-colors">
                          <Check size={12} />
                        </button>
                        <button onClick={() => setDeleteConfirm(null)} className="text-white p-1 hover:text-[#ff007a] transition-colors">
                          <X size={12} />
                        </button>
                      </div>
                    ) : (
                      <button onClick={() => setDeleteConfirm(rider.id)} className="text-[#444] hover:text-red-500 transition-all">
                        <Trash2 size={16} />
                      </button>
                    )}
                  </div>
        ...
        <div className="p-4 bg-[#141414] border-t border-[#222] flex justify-between items-center text-[10px] font-bold text-[#555] tracking-widest uppercase">
          ...
          <div className="flex gap-1">
            <button
              ...
              className="p-2 bg-[#1a1a1a] border border-[#222] rounded hover:border-[#ff007a] disabled:opacity-30 transition-all"
            >
              <ChevronLeft size={14} />
            </button>
            <button
      ...
      {(editingRider || isAdding) && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#111] border border-[#222] w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
            {editingRider && (
              <div className="w-full md:w-64 bg-[#0d0d0d] p-6 border-b md:border-b-0 md:border-r border-[#222]">
                <h3 className="text-[#ff007a] text-[10px] font-black uppercase tracking-widest mb-6 flex items-center gap-2 italic">
                ...
                <div className="space-y-4 text-[11px] text-[#444] font-medium uppercase tracking-tighter">
                  <div className="p-3 bg-[#ff007a]/5 border border-[#ff007a]/20 rounded-lg text-[#ff007a] italic">
                    ...
                    Înregistrat la: {new Date(editingRider.created_at).toLocaleDateString()}
                  </div>
            ...
            <div className="flex-1 p-8">
              <div className="flex justify-between items-start mb-8">
                <h2 className="text-xl font-black text-white uppercase tracking-tighter">
                  {isAdding ? 'Înregistrare Nouă' : 'Actualizare Cloud'}
                </h2>
                <button
                  onClick={() => { setEditingRider(null); setIsAdding(false); }}
                  className="text-[#444] hover:text-[#ff007a] transition-colors"
                >
                  <X size={24} />
                </button>
              </div>
              ...
              <form onSubmit={isAdding ? handleAdd : handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1">
                  ...
                  <input
                    className="w-full p-3 bg-[#0a0a0a] border border-[#222] rounded-lg text-sm focus:border-[#ff007a] outline-none text-white transition-all"
                    value={isAdding ? newRider.name : editingRider?.name}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    className="w-full p-3 bg-[#0a0a0a] border border-[#222] rounded-lg text-sm focus:border-[#ff007a] outline-none text-white transition-all"
                    type="email"
                    value={isAdding ? newRider.email : editingRider?.email}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      isAdding
                        ? setNewRider({ ...newRider, email: e.target.value })
                        : setEditingRider({ ...editingRider!, email: e.target.value })
                    }
                    required
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    className="w-full p-3 bg-[#0a0a0a] border border-[#222] rounded-lg text-sm focus:border-[#ff007a] outline-none text-white appearance-none"
                    value={isAdding ? newRider.preferred_payment : editingRider?.preferred_payment || 'card'}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      isAdding
                        ? setNewRider({ ...newRider, preferred_payment: e.target.value })
                        : setEditingRider({ ...editingRider!, preferred_payment: e.target.value })
                    }
                  >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    className="w-full p-3 bg-[#0a0a0a] border border-[#222] rounded-lg text-sm focus:border-[#ff007a] outline-none text-white transition-all"
                    value={isAdding ? newRider.phone : editingRider?.phone || ''}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      isAdding
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <textarea
                    className="w-full p-3 bg-[#0a0a0a] border border-[#222] rounded-lg text-sm focus:border-[#ff007a] outline-none min-h-[80px] resize-none text-white"
                    value={isAdding ? newRider.address : editingRider?.address || ''}
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                      isAdding
                        ? setNewRider({ ...newRider, address: e.target.value })
                        : setEditingRider({ ...editingRider!, address: e.target.value })
                    }
                  />
                </div>
                ...
                <div className="md:col-span-2 pt-4">
                  <button
                    type="submit"
                    className="w-full py-4 bg-[#ff007a] text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-xl hover:bg-[#d00063] transition-all shadow-lg shadow-[#ff007a]/20 flex items-center justify-center gap-2"
                  >
