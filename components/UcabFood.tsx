        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div className="flex items-center gap-6">
             ...
             <div>
                ...
                <p className="text-red-500 font-bold text-[10px] mt-2 tracking-[0.3em]">Meniu Restaurant</p>
             </div>
          </div>
          <button onClick={() => setShowMenuModal(true)} className="px-6 py-4 bg-white text-black rounded-2xl font-black text-[10px] tracking-widest hover:bg-red-600 transition-all">+ Preparat</button>
        </div>
          ...
          {menuItems.map((item) => (
            <div key={item.id} className="p-6 bg-[#0a0a0a] border border-white/5 rounded-[2rem] flex items-center justify-between group">
              <div className="flex items-center gap-4 text-xs">
                ...
                <div>
                  <p className="font-bold text-white tracking-tighter">{item.name}</p>
        ...
        {showMenuModal && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-6 italic uppercase font-black text-white">
            <div className="bg-[#080808] border border-white/10 p-10 rounded-[3rem] w-full max-w-md animate-in zoom-in-95">
              <h3 className="text-2xl font-black italic uppercase mb-8">Nou Preparat</h3>
              <form onSubmit={handleAddMenuItem} className="space-y-4">
                ...
                <input required placeholder="Nume" className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold" value={menuForm.name} onChange={e => setMenuForm({...menuForm, name: e.target.value})} />
                <input required placeholder="Preț" type="number" className="w-full bg-white/5 border border-white/10 p-5 rounded-2xl outline-none font-bold" value={menuForm.price} onChange={e => setMenuForm({...menuForm, price: e.target.value})} />
                <button type="submit" className="w-full bg-white text-black p-5 rounded-2xl font-black italic uppercase">Adaugă în Meniu</button>
