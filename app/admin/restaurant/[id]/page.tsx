      ...
      <header className="max-w-7xl w-full mx-auto flex flex-col lg:flex-row justify-between items-center mb-10 bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border-4 border-red-600 gap-8">
        ...
        <div className="flex gap-6 md:gap-10 border-t md:border-t-0 md:border-l-2 border-zinc-100 pt-6 md:pt-0 md:pl-10 w-full lg:w-auto justify-around lg:justify-end">
          <div className="text-center">
            <p className="text-[8px] md:text-[9px] text-zinc-400 mb-1 tracking-widest uppercase">Queue</p>
                  ...
                  <div key={order.id} className={`bg-white p-6 md:p-8 rounded-[3rem] shadow-xl border-2 transition-all ${order.status === 'pending' ? 'border-red-600 scale-105 z-10' : 'border-zinc-50'}`}>
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex-1">
                        <span className="text-[10px] text-red-600 font-black mb-1 block uppercase">#{order.id.slice(0, 8)}</span>
                        ...
                        <h3 className="text-xl md:text-2xl tracking-tighter font-black leading-tight text-zinc-900">{order.customer_name}</h3>
                      </div>
                      <button onClick={() => printReceipt(order)} className="p-3 bg-zinc-50 rounded-xl text-zinc-400 hover:text-red-600"><Printer size={20}/></button>
                    </div>
                    ...
                    <div className="flex items-end justify-between mb-8">
                       <div><p className="text-[8px] text-zinc-400 tracking-widest">TOTAL</p><p className="text-3xl text-red-600 font-black leading-none tracking-tighter">{order.total_amount} RON</p></div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      ...
                      {order.status === 'delivered' && <div className="text-center py-2 bg-green-50 text-green-600 rounded-xl text-[9px] font-black tracking-widest">✓ FINALIZATĂ</div>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border-4 border-zinc-100">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                      <tr className="text-[10px] tracking-widest text-zinc-400 border-b uppercase"><th className="p-8">PRODUS / DESCRIERE</th><th className="p-8 text-center">GRAMAJ</th><th className="p-8 text-center">PREȚ</th><th className="p-8 text-right">DISPONIBILITATE</th></tr>
                    </thead>
                    <tbody>
                      {menuItems.map((item) => (
                        <tr key={item.id} className={`border-b hover:bg-zinc-50 transition-colors ${!item.is_available ? 'bg-red-50/20' : ''}`}>
                          <td className="p-8"><div className="flex items-center gap-6"><img src={item.image_url} className={`w-16 h-16 rounded-2xl object-cover shadow-lg ${!item.is_available ? 'grayscale opacity-40' : ''}`} alt="" /><div><p className={`text-lg font-black leading-tight ${!item.is_available ? 'text-zinc-400 line-through' : 'text-zinc-900'}`}>{item.name}</p><p className="text-[10px] normal-case italic font-medium text-zinc-400 mt-1 line-clamp-1">{item.description || 'Fără descriere ANPC'}</p></div></div></td>
                          <td className="p-8 text-center text-zinc-400 font-black text-sm italic">{item.weight || 0} grame</td>
