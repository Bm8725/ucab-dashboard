...
export default function RestaurantLiveDash() {
  ...
  const printReceipt = (order: any) => {
    ...
    if (!w) return;
    w.document.write(`<body style="font-family:monospace;padding:20px;text-transform:uppercase;"><center><h2>UCAB FOOD</h2><h3>${restaurant?.name}</h3></center><hr/><p>CLIENT: ${order.customer_name}</p><p>TEL: ${order.customer_phone}</p><p>ADR: ${order.delivery_address || 'RIDICARE'}</p><hr/>${order.items.map((i: any) => `<p>${i.quantity}x ${i.name} - ${i.price}L</p>`).join('')}<hr/><p style="font-size:18px;font-weight:bold;">TOTAL: ${order.total_amount} RON</p><script>window.print();window.close();</script></body>`);
    w.document.close();
  };
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF7] text-zinc-900 p-4 md:p-12 font-sans italic uppercase font-black flex flex-col overflow-x-hidden">
      ...
      {/* AUDIO UNLOCK ALERT */}
      {!audioUnlocked && (
        <div className="fixed top-4 right-4 z-[200] bg-red-600 text-white px-4 py-2 rounded-full text-[10px] animate-bounce flex items-center gap-2 shadow-xl border-2 border-white cursor-pointer" onClick={() => setAudioUnlocked(true)}>
      ...
      <header className="max-w-7xl w-full mx-auto flex flex-col lg:flex-row justify-between items-center mb-10 bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border-4 border-red-600 gap-8">
        <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
          ...
          <div>
            ...
            <p className="text-xl md:text-2xl text-zinc-800 font-black">{restaurant?.name}</p>
            <p className="text-[10px] text-red-600 mt-2 flex items-center justify-center md:justify-start gap-1 font-bold italic"><MapPin size={12}/> {restaurant?.address || "LOCAȚIE ACTIVĂ"}</p>
          </div>
        </div>
        <div className="flex gap-8 border-t md:border-t-0 md:border-l-2 border-zinc-100 pt-6 md:pt-0 md:pl-10 w-full lg:w-auto justify-around">
          <div className="text-center">
            <p className="text-[8px] text-zinc-400 mb-1 tracking-widest uppercase">QUEUE</p>
          ...
          <div className="text-center">
            <p className="text-[8px] text-zinc-400 mb-1 tracking-widest uppercase">TOTAL L</p>
      ...
      <div className="max-w-7xl w-full mx-auto flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
        <nav className="flex gap-6 md:gap-10 border-b-2 border-zinc-100 w-full md:w-auto overflow-x-auto no-scrollbar px-2">
          ...
          <button onClick={() => setActiveTab('menu')} className={`pb-4 text-xs tracking-[0.3em] transition-all ${activeTab === 'menu' ? 'text-red-600 border-b-4 border-red-600' : 'text-zinc-300'}`}>MENIU & STOC</button>
        </nav>
        {activeTab === 'orders' && (
      ...
      <main className="max-w-7xl w-full mx-auto flex-1">
        <AnimatePresence mode="wait">
          {activeTab === 'orders' ? (
            <motion.div key="orders" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12">
                  ...
                  <div key={order.id} className={`bg-white p-6 md:p-8 rounded-[3rem] shadow-xl border-2 transition-all ${order.status === 'pending' ? 'border-red-600 scale-105 z-10' : 'border-zinc-50'}`}>
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex-1">
                        <span className="text-[9px] text-red-600 font-black mb-0.5 block tracking-widest">#{order.id.slice(0, 8)}</span>
                        {/* DATA PE CARD */}
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-6 py-12">
                  ...
                  <div className="text-center"><span className="text-[10px] text-zinc-300 tracking-widest block uppercase">PAGINA</span><span className="text-2xl font-black">{currentPage} / {totalPages}</span></div>
                    ...
                    <tbody>
                      {menuItems.map((item) => (
                        <tr key={item.id} className={`border-b transition-all ${!item.is_available ? 'bg-red-50/20' : 'hover:bg-zinc-50'}`}>
                          ...
                          <td className="p-8 text-right"><button onClick={() => toggleAvailability(item.id, item.is_available)} className={`px-6 py-3 rounded-2xl text-[9px] font-black tracking-widest transition-all ${item.is_available ? 'bg-zinc-900 text-white' : 'bg-red-600 text-white'}`}>{item.is_available ? <span className="flex items-center gap-1"><Eye size={14}/> ONLINE</span> : <span className="flex items-center gap-1"><EyeOff size={14}/> OFFLINE</span>}</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
