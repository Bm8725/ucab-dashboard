...
import { MapPin, Phone, CheckCircle2, Copy, Clock, Calendar, ChevronLeft, ChevronRight, BellRing, Utensils, Printer, Eye, EyeOff, Navigation, TrendingUp, DollarSign, ShoppingBag, Volume2 } from "lucide-react";
...
export default function RestaurantLiveDash() {
  ...
  useEffect(() => {
    // 1. INITIALIZARE AUDIO & UNLOCK LOGIC
    ...
    const content = `
      <html>
        <body style="font-family: 'Courier New', Courier, monospace; width: 280px; padding: 10px; text-transform: uppercase;">
          ...
          ${order.items?.map((i: any) => `<div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 5px;"><span>${i.quantity}x ${i.name}</span><span>${i.price}L</span></div>`).join('')}
          <div style="border-top: 2px dashed #000; margin-top: 10px; padding-top: 10px; display: flex; justify-content: space-between; font-weight: bold; font-size: 16px;"><span>TOTAL:</span><span>${order.total_amount} RON</span></div>
        </body>
      </html>`;
  ...
  const stats = useMemo(() => {
    const totalRevenue = filteredOrders.reduce((acc, curr) => acc + Number(curr.total_amount || 0), 0);
    const count = filteredOrders.length;
    const avgOrder = count > 0 ? (totalRevenue / count).toFixed(0) : 0;
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF7] text-zinc-900 p-4 md:p-8 lg:p-12 font-sans italic uppercase font-black flex flex-col overflow-x-hidden">
      ...
      {!audioUnlocked && (
        <div className="fixed top-4 right-4 z-[100] bg-red-600 text-white px-4 py-2 rounded-full text-[10px] animate-bounce flex items-center gap-2 shadow-2xl border-2 border-white">
           <Volume2 size={14} /> CLICK PENTRU SUNET ACTIV
        </div>
      )}
      ...
      <header className="max-w-7xl w-full mx-auto flex flex-col lg:flex-row justify-between items-center mb-10 bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border-4 border-red-600 gap-8">
        <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
          ...
          <div>
            <h1 className="text-4xl md:text-6xl lg:text-8xl tracking-tighter leading-none mb-2 uppercase text-zinc-900">UCAB <span className="text-red-600">FOOD</span></h1>
            ...
            <p className="text-[9px] md:text-[10px] text-red-600 tracking-[0.2em] mt-2 flex items-center justify-center md:justify-start gap-2 italic">
              <MapPin size={12} strokeWidth={3} /> {restaurant?.address || "ADRESA UCAB"}
            </p>
          </div>
        </div>
        <div className="flex gap-10 border-t lg:border-t-0 lg:border-l-2 border-zinc-100 pt-6 lg:pt-0 lg:pl-10 w-full lg:w-auto justify-around">
          <div className="text-center">
            <p className="text-[8px] text-zinc-400 mb-1 tracking-widest uppercase">Revenue</p>
            <p className="text-4xl md:text-6xl text-zinc-900 font-black leading-none">{stats.totalRevenue.toFixed(0)}</p>
          </div>
          <div className="text-center">
            <p className="text-[8px] text-zinc-400 mb-1 tracking-widest uppercase">Orders</p>
            <p className="text-4xl md:text-6xl text-red-600 font-black leading-none">{stats.count}</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl w-full mx-auto flex flex-col md:flex-row justify-between items-center mb-8 gap-6">
        <nav className="flex gap-6 md:gap-10 border-b-2 border-zinc-100 w-full md:w-auto overflow-x-auto no-scrollbar">
          ...
          <button onClick={() => setActiveTab('menu')} className={`pb-4 md:pb-6 text-xs md:text-sm tracking-[0.2em] md:tracking-[0.4em] whitespace-nowrap transition-all ${activeTab === 'menu' ? 'text-red-600 border-b-4 border-red-600' : 'text-zinc-300'}`}>Meniu & Stoc</button>
        </nav>
        {activeTab === 'orders' && (
          <div className="flex bg-white p-2 rounded-2xl shadow-lg border border-zinc-50">
            {['today', 'yesterday', 'all'].map((f) => (
              <button key={f} onClick={() => {setDateFilter(f as any); setCurrentPage(1);}} className={`px-4 py-2 rounded-xl text-[9px] tracking-widest transition-all ${dateFilter === f ? 'bg-red-600 text-white' : 'text-zinc-300'}`}>{f.toUpperCase()}</button>
            ))}
          </div>
        )}
      </div>
      ...
      <main className="max-w-7xl w-full mx-auto flex-1">
        <AnimatePresence mode="wait">
          {activeTab === 'orders' ? (
            <motion.div key="orders" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-10">
              
              {/* STATS STRIP */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white p-6 rounded-[2rem] border border-zinc-100 flex items-center gap-4">
                  <div className="p-4 bg-zinc-50 rounded-2xl text-red-600"><TrendingUp size={24}/></div>
                  <div><p className="text-[8px] text-zinc-400 tracking-widest uppercase">Avg Order</p><p className="text-xl font-black">{stats.avgOrder} L</p></div>
                </div>
                <div className="bg-white p-6 rounded-[2rem] border border-zinc-100 flex items-center gap-4">
                  <div className="p-4 bg-zinc-50 rounded-2xl text-zinc-900"><DollarSign size={24}/></div>
                  <div><p className="text-[8px] text-zinc-400 tracking-widest uppercase">Net Sales</p><p className="text-xl font-black">{stats.totalRevenue} L</p></div>
                </div>
                <div className="bg-white p-6 rounded-[2rem] border border-zinc-100 flex items-center gap-4">
                  <div className="p-4 bg-zinc-50 rounded-2xl text-zinc-900"><ShoppingBag size={24}/></div>
                  <div><p className="text-[8px] text-zinc-400 tracking-widest uppercase">Total Orders</p><p className="text-xl font-black">{stats.count} QTY</p></div>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {currentOrders.map((order) => (
                  <div key={order.id} className={`bg-white p-8 rounded-[3.5rem] shadow-xl border-2 transition-all ${order.status === 'pending' ? 'border-red-600 scale-105 z-10' : 'border-zinc-50'}`}>
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex-1">
                        <span className="text-[9px] text-red-600 font-black mb-1 block uppercase">#{order.id.slice(0, 8)}</span>
                    ...
                    <div className="bg-zinc-50/50 p-4 rounded-2xl mb-6 space-y-2 border border-zinc-100">
                      <p className="text-[10px] flex items-center gap-2 font-black text-zinc-600 uppercase"><Phone size={12} className="text-red-600"/> {order.customer_phone}</p>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {order.status === 'pending' && <button onClick={() => updateStatus(order.id, 'preparing')} className="bg-zinc-900 text-white py-4 rounded-2xl text-[10px] font-black hover:bg-red-600 transition-all uppercase">ACCEPTĂ</button>}
                      {order.status === 'preparing' && <button onClick={() => updateStatus(order.id, 'delivered')} className="bg-red-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase">FINALIZATĂ</button>}
            ...
            <motion.div key="menu" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pb-20">
      ...
      {/* BRANDING FOOTER */}
      <footer className="max-w-7xl w-full mx-auto pt-12 pb-8 border-t-2 border-zinc-100 mt-auto opacity-40 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center gap-3">
          <img src="/ucabfood.png" className="w-8 h-8 grayscale" alt="Logo" />
