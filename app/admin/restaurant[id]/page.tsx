...
import { ChevronLeft, MapPin, Phone, Trash2, Zap } from "lucide-react";

export default function RestaurantLiveDash() {
  const { id } = useParams();
  const router = useRouter();
  
  // FIX TIPURI PENTRU BUILD
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`live-${id}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "orders" }, (payload: any) => {
  ...
  // FIX TIPURI ARGUMENTE
  const updateStatus = async (orderId: any, status: string) => {
    await supabase.from("orders").update({ status }).eq("id", orderId);
    setOrders((prev: any[]) => prev.map(o => o.id === orderId ? { ...o, status } : o));
  };

  if (loading) return (
    <div className="h-screen bg-black flex items-center justify-center text-red-600 font-black italic tracking-[0.5em] uppercase">
      Connecting to Hub...
    </div>
  );
    ...
    <div className="min-h-screen bg-[#020202] text-white p-8 md:p-12 font-sans italic uppercase">
      <header className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-8">
        <div className="flex items-center gap-8">
          <button onClick={() => router.back()} className="p-5 bg-white/5 border border-white/10 rounded-3xl hover:bg-red-600 transition-all">
            <ChevronLeft/>
          </button>
          <div>
        ...
        <div className="bg-white/5 p-8 rounded-[3rem] border border-white/10 flex gap-10">
            <div className="text-center">
              <p className="text-[9px] font-black text-zinc-600 mb-1">Incoming</p>
              <p className="text-3xl font-black text-red-600">{orders.filter((o: any) => o.status === 'pending').length}</p>
            </div>
            <div className="text-center">
              <p className="text-[9px] font-black text-zinc-600 mb-1">Revenue Today</p>
              <p className="text-3xl font-black">{orders.reduce((a: number, c: any) => a + (c.total_amount || 0), 0).toFixed(0)} <span className="text-xs">RON</span></p>
            </div>
        </div>
      </header>
                ...
                <div className="bg-white/5 rounded-[2rem] p-6 mb-8 border border-white/5 space-y-2 font-black italic text-[11px]">
                  {order.items?.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between">
                      ...
                      <span className="text-zinc-600">{item.price} RON</span>
                    </div>
                  ))}
                </div>
